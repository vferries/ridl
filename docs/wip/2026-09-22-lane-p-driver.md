...
- Every GitHub comment you post ends with a blank line, a `---` line, and one
  ...
  driftsys/ridl#328 does. No model identifier in any file, commit message or
  pull request text.
...
Then a second pass over the fix commits alone. CI green is not review; the seats
are.
...
- The code: `crates/ridl-ir/proto/ridl/ir/v2/ir.proto` and
  ...
  `generate(&v2::Package)` and `generate_with(package, others)` in
  `ridl-backend-proto`, `ridl-backend-flatbuffers`, `ridl-backend-ts`, and in
  `ridl-backend-rust` also `generate_face_with` and `generate_pipeline`;
  `crates/ridl-ir/src/name.rs` and `src/projection/flatbuffers.rs` (facts
  already shared between backends, the precedent for the lowered model);
  `crates/ridlc/tests/corpus.rs` and its snapshots (byte-identity is measured
  against these); `xtask/src/codegen.rs` (the pattern for generated code checked
  in with a staleness test).
...
- Kotlin is the first external plugin. Its IPC binding is AIDL over Binder on
  Android, generated by the Kotlin backend from the same lowered model as its
  types and faces. It does not use the WebSocket transport (E11.9). It needs the
  logical frame (E11.1) to know what crosses a boundary per interaction kind.
...
coordination issue): the merge SHA, what landed, what another lane must know.
...
every `--emit ir-json`, `ir-txtpb`, `ir-binpb` snapshot, unless the disposition
changed the canonical form, in which case each moved snapshot is read before
re-accepting.
...
- `docs/book/cli-reference.md` gains the flag and the plugin lookup rule;
  `docs/design/` gains `codegen-plugins.md`, the as-built record; #322's
  `Done when` on the roadmap is corrected per D-P1.
...
host needs a capability ADR-0020 decision 10 rules out (a plugin reading or
